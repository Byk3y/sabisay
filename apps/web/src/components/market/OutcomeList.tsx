...
export const OutcomeList = ({ 
  ...
  return (
    <div className="mb-2">
      <div className={`${isMobile ? 'flex justify-between items-center' : 'flex items-center'} pb-3 border-b border-gray-200 dark:border-gray-700 mb-0`}>
        <div className="flex-1">
          <h2 className={`${isMobile ? 'text-xs' : 'text-sm'} font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide`}>
            OUTCOME
          </h2>
        </div>
        <div className={`${isMobile ? 'flex items-center gap-2' : 'flex-1 flex justify-center'}`}>
          <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide`}>
            % CHANCE
          </span>
      ...
      <div className={`${isMobile ? 'space-y-0' : 'divide-y divide-gray-200 dark:divide-gray-700'}`}>
        {outcomes.map((outcome, index) => (
          <div key={index} className={`${isMobile ? 'py-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0' : 'flex items-center py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors'}`}>
            {isMobile ? (
                ...
                {/* Candidate info row */}
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-600 dark:text-gray-300`}>
        ...
        {isMobile && (
          <div className="py-4">
            <h3 className="font-bold text-gray-900 dark:text-white text-base mb-3">About</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-900 dark:text-white">${formatCurrencyNoSymbol(outcomes[0]?.volume || 0)}</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-5 h-5 text-blue-500">
                ...
                <span className="text-sm font-medium text-gray-900 dark:text-white">Feb 25, 2027</span>
              </div>
            </div>
          </div>
        )}
