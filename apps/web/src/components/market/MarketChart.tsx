...
export const MarketChart = ({
  ...
  return (
    <div className={`mb-6 ${isMobile ? '-mt-1' : '-mt-6'}`}>
      {/* Chart Area */}
      <div
        className={`relative ${isMobile ? 'h-[145px]' : 'h-60'} flex items-center justify-center pr-8`}
      >
            ...
            <div className="flex-1 h-full relative">
              ...
              <div className="absolute -left-4 right-6 top-0 bottom-0 flex flex-col justify-between">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    key={i}
                    className="border-t border-dotted border-gray-300 dark:border-gray-600 w-full"
                  ></div>
                ))}
              </div>
              ...
              <svg
                className="absolute inset-0 w-[calc(100%-2rem)] h-full"
                viewBox="0 0 750 240"
              >
