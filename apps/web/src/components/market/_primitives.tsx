...
export function RowLine({
  ...
  return (
    <div
      className={`flex items-center justify-between gap-3 py-1 ${showDivider ? 'border-b border-gray-200 dark:border-gray-700' : ''}`}
    >
...
export function ActionPill({
  ...
  const baseClasses =
    'rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors shadow-sm';
  const yesClasses =
    'bg-green-100 hover:bg-green-200 dark:bg-green-500/20 dark:hover:bg-green-500/30 border border-green-300 dark:border-green-500/30 text-green-800 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300';
  const noClasses =
    'bg-red-100 hover:bg-red-200 dark:bg-red-500/20 dark:hover:bg-red-500/30 border border-red-300 dark:border-red-500/30 text-red-800 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300';
