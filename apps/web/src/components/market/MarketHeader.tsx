...
export const MarketHeader = ({ market, onShare, onBookmark, isMobile = false, isBinaryMarket = false }: MarketHeaderProps) => {
  return (
    <div className="mb-4">
      ...
      {isMobile && (
        <div className="flex items-center justify-between -mt-3 bg-transparent">
          ...
          <div className="text-sm text-gray-500 dark:text-gray-500">
            <span className="font-normal">${formatCurrencyNoSymbol(market.volume)} Vol.</span>
          </div>
      ...
      <div className={`flex items-center gap-4 ${isMobile ? 'mb-4 mt-3' : 'mb-4'}`}>
        <div className={`${isMobile ? 'w-12 h-12 rounded-lg' : 'w-16 h-16 rounded-xl'} bg-gray-200 dark:bg-gray-700 flex items-center justify-center`}>
          <span className={`text-gray-500 dark:text-gray-400 ${isMobile ? 'text-lg' : 'text-3xl'} font-bold`}>📊</span>
        </div>
        <div className="flex-1">
          <h1 className={`${isMobile ? 'text-lg' : 'text-3xl'} font-bold text-gray-900 dark:text-white`}>
            {market.title}
          </h1>
        </div>
