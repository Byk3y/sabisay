...
import { cleanTradeAmountInput, isTradeAmountWithinLimits } from '@/lib/tradingUtils';
...
export interface UseAmountInputReturn {
  ...
  handleFocus: () => void;
  handleBlur: () => void;
  handleChange: (value: string, onAmountChange: (amount: string) => void) => void;
  handleKeyDown: (e: React.KeyboardEvent<HTMLInputElement>) => void;
  handleClick: (e: React.MouseEvent<HTMLInputElement>) => void;
  handleSelect: (e: React.SyntheticEvent<HTMLInputElement>) => void;
  handleIncrement: (currentAmount: string, onAmountChange: (amount: string) => void) => void;
  handleDecrement: (currentAmount: string, onAmountChange: (amount: string) => void) => void;
...
export const useAmountInput = (): UseAmountInputReturn => {
  ...
  const handleChange = useCallback((value: string, onAmountChange: (amount: string) => void) => {
  ...
  const handleIncrement = useCallback((currentAmount: string, onAmountChange: (amount: string) => void) => {
  ...
  const handleDecrement = useCallback((currentAmount: string, onAmountChange: (amount: string) => void) => {
    const current = parseFloat(currentAmount) || 0;
